...
const OSS_TOKEN_URL = 'https://security.dev.quancheng-ec.com/oss/getOssStsToken'
...
class Client extends EventEmitter {
  ...
  async uploadFiles(bucket, options) {
    try {
      let { tag, localFileDirectory, localFile, uploadFilePath } = options
      // 初始化client
      let ossClient = await this.getClient(bucket, options)
      // 获取文件路径
      let localFiles = []
      // 获取文件夹下的所有文件
      if (localFileDirectory) localFiles.push(...findFilesByPath(localFileDirectory))
      // 获取指定文件
      if (localFile) {
        if (uploadFilePath) {
          let fileSplit = localFile.split('/')
          let uploadDir = path.join(tag || '', uploadFilePath || '', fileSplit[fileSplit.length - 1])
          console.log('正在上传 %s  =>   %s', localFile, uploadDir)
          await ossClient.put(uploadDir, localFile)
        } else {
          localFiles.push(localFile)
        }
      }
      if (!localFiles || !localFiles.length) {
        return this.emit('error', `没有需要上传的文件: -l ${localFileDirectory}   -f ${localFile}`)
      }
      ...
      await localFiles.forEach(async (lf, i) => {
        let uploadDir = path.join(tag || '', uploadFilePath || '', lf.replace(localFileDirectory, ''))
        console.log('正在上传 %s  =>   %s', lf, uploadDir)
        await ossClient.put(uploadDir, lf)
      })
      console.log(`-----  上传成功  -----`)
      console.log(`${bucket}.${ossClient.options.endpoint.host}`)
    } catch (error) {
      this.emit('error', error)
    }
  }
